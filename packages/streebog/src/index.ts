...
export class Streebog {
  ...
  public digestInto(buf: Uint8Array): Uint8Array {
    ...
    const msg: Uint8Array = message.slice(
      0,
      message.length - (blocks - 1) * 64
    );
    ...
    hash = transformG(
      ...
      transformG(
        new Uint8Array(64),
        transformG(n, hash, paddedMsg),
        add512(n, msgLen.reverse())
      ),
...
export const streebog256 = (input: Uint8Array): Uint8Array =>
  new Streebog256().update(input).digest();
...
export const streebog512 = (input: Uint8Array): Uint8Array =>
  new Streebog512().update(input).digest();
